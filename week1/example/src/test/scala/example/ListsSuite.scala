 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a list with repeated values") {
    assert(sum(List(2,5,2,0)) === 9)
  }
  ...
  test("max of list with negative numbers") {
    assert(max(List(-3, 2, 1)) === 2)
  }

  test("max of an empty list should return exception") {
    intercept[java.util.NoSuchElementException] {
      max(List())
    }
  }

  test("max on list with only one element") {
    assert(max(List(1)) === 1)
  }
